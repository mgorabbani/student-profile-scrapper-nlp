...
var target = `VISA Interview Experience: USA
...
var BSC = 'B.Sc';
console.log(natural.LevenshteinDistance(BSC, target, { search: true }));
let result = []
result.push(nlp(target).match('GRE #Value').out('text'))
result.push(nlp(target).match('Publication #Value').out('text'))
result.push(nlp(target).match('TOEFL #Value').out('text'))
result.push(nlp(target).after(BSC + '#Value').out('text'))
console.log(result);
